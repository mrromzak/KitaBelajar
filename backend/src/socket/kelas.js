...
module.exports = function (io) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('kelas:chat', ({ kelasId, id, isi, pengirim }) => {
      if (!socket.user?.id && !connectedUsers[socket.id]) return;
      const userInfo = connectedUsers[socket.id];
      if (userInfo && userInfo.kelasId !== kelasId) return;
    ...
    socket.on('kelas:edit_pesan', ({ kelasId, msgId, isi }) => {
      io.to('kelas:' + kelasId).emit('kelas:pesan_diedit', { msgId, isi: cleanText(isi || '', 5000) });
    });
    ...
    socket.on('private:send', (payload) => {
      ...
      if (toUserId) {
        io.to('user:' + toUserId).emit('private:receive', {
          ...payload,
          isi: cleanText(payload.isi || '', 5000),
          pengirim_nama: cleanText(payload.pengirim_nama || '', 100)
        });
      }
    });
